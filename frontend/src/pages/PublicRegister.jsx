...
import { Upload, FileCheck, CheckCircle2, ShieldAlert } from 'lucide-react';
...
export const PublicRegister = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        if (age < 17) {
            setError('Usia minimal harus 17 tahun untuk mendaftar.');
            setSubmitting(false);
            return;
        }
        ...
        if (!/^\+?[0-9]{10,15}$/.test(form.phone)) {
            setError('Nomor HP tidak valid (harus 10-15 digit angka).');
            setSubmitting(false);
            return;
        }
        ...
        if (!files.installation) {
            setError('Foto titik instalasi wajib diunggah.');
            setSubmitting(false);
            return;
        }
